...
export const VAutoCompleteStudent: React.FC<VAutoCompleteTeacherProps> = ({ isExternalLoading = false }) => {
    const { fieldName, registerField, defaultValue, error, clearError } = useField('idStudent')
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const result = await UserService.listUser(1, '', 'desc', true, 'STUDENT')
            setIsLoading(false)
